...
It also exposes a "browser" member (i.e. `process.browser`) which is `true` in this implementation but `undefined` in node. This can be used in Aeristo code that adjusts it's behavior depending on which environment it's running in. 
